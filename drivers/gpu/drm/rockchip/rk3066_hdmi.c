...
#define RK3066_GRF_SOC_CON0		0x150
#define RK3066_HDMI_LCDC_SEL		BIT(14)
...
static void rk3066_hdmi_encoder_enable(struct drm_encoder *encoder)
{
	...
	if (ret)
		val = RK3066_HDMI_LCDC_SEL | (RK3066_HDMI_LCDC_SEL << 16);
	else
		val = RK3066_HDMI_LCDC_SEL << 16;

	ret = regmap_write(hdmi->regmap, RK3066_GRF_SOC_CON0, val);
	if (ret != 0)
		DRM_DEV_ERROR(hdmi->dev, "Could not write to GRF: %d\n", ret);

	DRM_DEV_DEBUG(hdmi->dev, "vop %d output to hdmi\n", ret);
